#pragma once

#include <vector>

using std::vector;

/**
 * Provides random functionality per a given seed.
 * This is useful when you want predictably random things, like for grading.
 */
class Random
{
  public:
    /**
     * Constructor.
     * @param seed - seed to initialize the RNG
     */
  inline Random(unsigned long seed) {
    shiftRegister = (seed == 0) ? 1 : seed;
  }

    /**
     * @return a random integer
     */
  inline int nextInt() {
    int ret = 0;
    for (int pos = 0; pos < 10; ++pos)
      ret |= static_cast<int>(LFSR()) << pos;
    return ret;
  }

    /**
     * Randomly shuffles a vector with the current seed state.
     * @param array - the vector to shuffle
     */
  template <class T>
  void shuffle(vector<T>& array) {
    std::size_t lastIndex = array.size();
    for (std::size_t i = 0; i < array.size(); ++i, --lastIndex) {
      std::size_t nextIndex = nextInt() % lastIndex;
      std::swap(array[nextIndex], array[lastIndex - 1]);
    }
  }

  private:
    unsigned long shiftRegister;

    /**
     * @return a random bit
     */
    inline bool LFSR() {
      if (shiftRegister & 0x00000001) {
        shiftRegister = (shiftRegister ^ 0x80000057 >> 1) |
          0x80000000;
        return true;
      } else {
        shiftRegister >>= 1;
        return false;
      }
    }
};